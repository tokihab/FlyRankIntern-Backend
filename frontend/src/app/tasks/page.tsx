...
export default function TasksPage() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-10">
      <Header title="Tasks & Auth" subtitle="Supabase-protected identity and the original SQLite CRUD surface." />
      <div className="grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-6">
          <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Session boundary</p>
          <h2 className="mt-4 text-xl font-medium">Supabase Auth</h2>
          {token ? (
            <div className="mt-5 flex flex-wrap items-center gap-3">
              <span className="rounded-full bg-emerald-300/15 px-3 py-1 text-xs text-emerald-200">Session Active</span>
              <span className="text-sm text-slate-300">{email}</span>
              <Button
                variant="outline"
                size="sm"
                onClick={logout}
                className="text-rose-200 border-rose-400/15"
              >
            ...
            <div className="mt-4 space-y-2">
              <input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Email" type="email" className="w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-sm" />
              <input value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Password" type="password" className="w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-sm" />
        ...
        <div className={`relative rounded-2xl border border-white/10 bg-white/[0.04] p-6 ${!token ? "overflow-hidden" : ""}`}>
          {!token && (
            <div className="absolute inset-0 z-10 flex items-center justify-center bg-slate-950/75 p-6 text-center backdrop-blur-sm">
              <span className="rounded-lg border border-amber-300/20 bg-amber-300/10 px-4 py-3 text-sm text-amber-100">
                Authentication required to view and manage tasks.
              </span>
            </div>
          )}
          <div className={!token ? "blur-sm" : ""}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.2em] text-slate-500">Task database</p>
                <h2 className="mt-2 text-xl font-medium">SQLite CRUD</h2>
              </div>
              <Button
                variant="outline"
                size="icon"
            ...
            <div className="mt-5 space-y-2">
              {tasks.map((task) => (
                <div key={task.id} className="flex items-center justify-between rounded-lg border border-white/10 px-3 py-3 text-sm">
                  <span>{task.title}</span>
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => void remove(task.id)}
                    className="text-slate-500 hover:text-rose-300 h-6 w-6"
              ...
              {tasks.length === 0 && (
                <p className="py-6 text-center text-sm text-slate-500">No tasks returned.</p>
              )}
            </div>
          </div>
        </div>
